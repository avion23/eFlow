		...
		if (profileZone == 0) {
			...
			if (processTempReached && processTimer >= startup_sec) {
				...
				if (processTimer >= startup_sec) {
					systemMessage =
							"Unable to reach process startup temperature within given time! Aborting.";
					processStop();
					return;
			...
			if (Input + processTempHysteresis
					>= reflowProfile[0].profileRamp[1]) {
				processTempReached = 1;
			}
			...
			if (Input + processTempHysteresis
					>= reflowProfile[0].profileRampToPeak[1]) {
				processTempReached = 1;
			}
			...
			if ((Input + 4) >= reflowProfile[0].profileReflow[1]
					&& processReflowTimer > zoneTime) {
				profileZone = 5;
				processReflowInitTimer = 0;
			}
			...
			if ((Input + 1) >= reflowProfile[0].profileCooling[1]
					&& processReflowTimer > zoneTime) {
				profileZone = 6;
			}
			...
			if ((Input + 3) >= reflowProfile[0].profileFinishing[1]
					&& processReflowTimer > zoneTime) {
				profileZone = 7;
			}
...
//  if (safeTemperature < ((sensorA + sensorB) / 2)) {
	if (safeTemperature < sensorA) {
		systemMessage =
				"Initialization aborted. Oven is warmer than the safe temperature.";
		return;
	}
